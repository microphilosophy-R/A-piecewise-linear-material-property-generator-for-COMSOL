...
class PiecewisePropertyGenerator():
    ...
    def _get_property_method(self, prop_name):
        ...
        method_map = {
            ...
            'enthalpy': self.state.hmass,
            'entropy': self.state.smass,
...
p = PiecewisePropertyGenerator('PARAHYD', 'REFPROP', 20, 350,
                               2e6, 0.2, property_func='enthalpy',
                               output_dir=r"C:\本地文件-COMSOL建模\正仲氢分段热力学性质\thermal_properyoes(0704)")

p.run()
# hydrogen, PARAHYD
p.generate_all_properties('PARAHYD', 'REFPROP', 20, 350,
                          2e6, 0.2,
                          output_dir=r"C:\本地文件-COMSOL建模\正仲氢分段热力学性质\thermal_properyoes(0704)")
